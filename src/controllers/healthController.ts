...
export const createHealthController = (service: TreeSitterService) => {
  ...
  const basicHealthCheck = async (
    ...
    try {
      log.debug(
        'HealthController',
        `Basic health check - RequestID: ${requestId}`,
      );
      ...
      log.error(
        'HealthController',
        `Basic health check failed - RequestID: ${requestId}, Error: ${errorMessage}`,
      );
  ...
  const detailedHealthCheck = async (
    ...
    try {
      log.debug(
        'HealthController',
        `Detailed health check - RequestID: ${requestId}`,
      );
      ...
      // 根据健康状态设置HTTP状态码
      const statusCode =
        detailedStats.health.status === 'error'
          ? 503
          : detailedStats.health.status === 'warning'
            ? 200
            : 200;
      ...
      log.error(
        'HealthController',
        `Detailed health check failed - RequestID: ${requestId}, Error: ${errorMessage}`,
      );
  ...
  const memoryUsage = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      log.debug(
        'HealthController',
        `Memory usage check - RequestID: ${requestId}`,
      );
      ...
      log.error(
        'HealthController',
        `Memory usage check failed - RequestID: ${requestId}, Error: ${errorMessage}`,
      );
  ...
  const serviceStats = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      log.debug(
        'HealthController',
        `Service stats check - RequestID: ${requestId}`,
      );
      ...
      log.error(
        'HealthController',
        `Service stats check failed - RequestID: ${requestId}, Error: ${errorMessage}`,
      );
  ...
  const triggerCleanup = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      log.info(
        'HealthController',
        `Triggering cleanup - RequestID: ${requestId}, Strategy: ${strategy}`,
      );
      ...
      log.error(
        'HealthController',
        `Cleanup trigger failed - RequestID: ${requestId}, Error: ${errorMessage}`,
      );
  ...
  const resetStats = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      log.error(
        'HealthController',
        `Stats reset failed - RequestID: ${requestId}, Error: ${errorMessage}`,
      );
